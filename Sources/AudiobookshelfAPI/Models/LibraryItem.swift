...
public struct LibraryItem {
    ...
    /// The time (in ms since POSIX epoch) when the library item was last scanned. Will be null if the server has not
    /// yet scanned the library item.
    ...
    /// If progress was requested, the user's progress for the book or podcast episode. If no progress exists, neither
    /// will this attribute.
    public let userMediaProgress: MediaProgress?

    /// If rssfeed was requested, the open RSS feed of the library item. Will be null if the RSS feed for this library
    /// item is closed.
...
extension LibraryItem: Decodable {
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.episodesDownloading = try container.decodeIfPresent(
            [PodcastEpisodeDownload].self,
            forKey: .episodesDownloading
        )
        self.recentEpisode = try container.decodeIfPresent(PodcastEpisode.self, forKey: .recentEpisode)
        self.prevBookInProgressLastUpdate = try container.decodeIfPresent(
            Int.self,
            forKey: .prevBookInProgressLastUpdate
        )
