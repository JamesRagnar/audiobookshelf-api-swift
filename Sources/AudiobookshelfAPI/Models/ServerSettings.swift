...
public struct ServerSettings {
    ...
    /// Whether the scanner will attempt to find a cover if your audiobook does not have an embedded cover or a cover
    /// image inside the folder. Note: This will extend scan time.
    ...
    /// Whether to extract subtitles from audiobook folder names. Subtitles must be separated by -, i.e.
    /// /audiobooks/Book Title - A Subtitle Here/ has the subtitle A Subtitle Here.
    public let scannerParseSubtitle: Bool

    /// Whether matched data will override item details when using Quick Match. By default, Quick Match will only fill
    /// in missing details.
    public let scannerPreferMatchedMetadata: Bool

    /// Whether to disable the automatic adding/updating of items when file changes are detected. Requires server
    /// restart for changes to take effect.
    public let scannerDisableWatcher: Bool

    /// Whether to store covers in the library item's folder. By default, covers are stored in /metadata/items. Only one
    /// file named cover will be kept.
    public let storeCoverWithItem: Bool

    /// Whether to store metadata files in the library item's folder. By default, metadata files are stored in
    ...
    /// Whether to ignore prefixes when sorting. For example, for the prefix the, the book title The Book Title would
    /// sort as Book Title, The.
    ...
    /// What date format to use. Options are MM/dd/yyyy, dd/MM/yyyy, dd.MM.yyyy, yyyy-MM-dd, MMM do, yyyy, MMMM do,
    /// yyyy, dd MMM yyyy, or dd MMMM yyyy.
